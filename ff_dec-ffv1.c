...
GF_FilterRegister FFDecodeRegister = {
	...
	GF_FS_SET_DESCRIPTION("FFMPEG FFV1 decoder")
	GF_FS_SET_HELP("This filter decodes FFV1, the lossless intra-frame video codec, using a reduced FFMPEG build carrying that decoder only."
	)
...
const GF_FilterRegister * EMSCRIPTEN_KEEPALIVE ffdec_ffv1_register(GF_FilterSession *session)
{
...
const GF_FilterRegister *ffdec_ffv1_register(GF_FilterSession *session)
{
	return NULL;
}
#endif
...
void register_ffdec_ffv1(void) {
    gf_filter_auto_register("ffdec_ffv1", ffdec_ffv1_register);
}
